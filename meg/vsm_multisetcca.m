function [W, A, rho, Rtest, Xtest, testfold] = vsm_multisetcca(X,nfold,K,lambda,shufflag,stratified)
...
st = dbstack;
recursivecall = numel(st)>1 && sum(strcmp({st.name},'vsm_multisetcca'))>1;

testfold = nfold;
if (numel(nfold)==1 && nfold>1 && ~recursivecall) || iscell(nfold)
  ...
  for k = 1:nfold
    ...
    [W(:,:,:,k), A(:,:,:,k), rho(:,:,k), Rtest(:,:,k), Xtest(k,:)] = vsm_multisetcca(X,testfold{k},K,lambda,shufflag);
  end
...
computeRtest = false;
if numel(nfold)>1 || recursivecall
  indx_test  = nfold;
