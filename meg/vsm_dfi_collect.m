...
for k = 1:numel(d)
  ...
  % smooth a bit, to account for interindividual difference + reduce noise
  krn = shiftdim([0.25 0.5 0.25;0.5 1 0.5;0.25 0.5 0.25],-2);
  krn = krn./sum(krn(:));
  dfi.dfi   = convn(dfi.dfi,krn,   'same');
  dfi_c.dfi = convn(dfi_c.dfi,krn, 'same');
...
siz = size(D);
Dc(:,:,1:size(D,3),:) = repmat(nanmean(D,3),[1 1 size(D,3) 1]);
...
tim   = dfi.time;
save('stats_dfi_audio', 'p', 'T', 'sel', 'indx', 'tim', 'label');
